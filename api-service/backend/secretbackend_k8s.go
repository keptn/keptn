...
import (
	...
	"github.com/keptn/keptn/api-service/common"
...
const SecretServiceName = "keptn-api-service"
...
func (k K8sSecretBackend) createK8sRoleBindingObj(secret model.Secret, roles []rbacv1.Role, namespace string) rbacv1.RoleBinding {
	...
	roleBinding := rbacv1.RoleBinding{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: namespace,
			Labels: map[string]string{
				"app.kubernetes.io/managed-by": SecretServiceName, // add a 'managed-by' label so we can identify secrets managed by the api-service
				"app.kubernetes.io/scope":      secret.Scope,
			},
		},
