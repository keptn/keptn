...
export class ForwarderGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot) {
    ...
    if (route.params.eventId) { // project/:projectName/:serviceName/:contextId/:eventId
      this.router.navigate(['project', route.params.projectName, 'sequence', route.params.contextId, 'event', route.params.eventId]);
    } else if (route.params.contextId) { // project/:projectName/:serviceName/:contextId
      this.router.navigate(['project', route.params.projectName, 'sequence', route.params.contextId]);
    } else { // project/:projectName/:serviceName
