import { EventTypes } from '../interfaces/event-types';
import { IProject } from '../interfaces/Project';

const keptnContextDeploymentDev = '2e21574c-dcf7-4275-b677-6bc19214acd5';
const keptnContextDeploymentStaging = '29af69cc-ea85-4358-b169-ce29034d9c81';
const devCartsDeploymentFinished = {
  eventId: '88c5b44b-9b97-4ffb-9349-f0e431efe519',
  keptnContext: keptnContextDeploymentDev,
  time: '1636114893991052283',
};

const devCartsEvaluationFinished = {
  eventId: '16338541-30a0-4a6b-b990-d4e81d0dd9fd',
  keptnContext: '79a80508-b9c5-4b71-a8c7-98b39bb4b5dd',
  time: '1635866557202816616',
};

const devCartsDbDeploymentFinished = {
  eventId: '04e84e11-40ac-446b-abc3-31844b8b4307',
  keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
  time: '1634037198563544316',
};

const stagingCartsDeploymentFinished = {
  eventId: 'cd2bed17-388d-4771-a95e-0548093ae8b8',
  keptnContext: keptnContextDeploymentStaging,
  time: '1636109341288361706',
};

const stagingCartsEvaluationFinished = {
  eventId: '7c1e8531-3005-4b4f-831e-da84d3613f93',
  keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
  time: '1636109551167902030',
};

const stagingCartsDbDeploymentFinished = {
  eventId: '84c9d335-508c-4e26-8947-c2246c54ac03',
  keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
  time: '1634037239263283210',
};

const productionCartsDeploymentFinished = {
  eventId: '9998714f-64f1-431f-8423-7bbcef957604',
  keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
  time: '1634122878567406201',
};

const productionCartsDbDeploymentFinished = {
  eventId: '2ce9d439-6c75-44d0-aa3f-6f2b3576f8e7',
  keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
  time: '1634037297469702061',
};

const devCartsLatestServiceEvent = {
  eventId: '1064035a-a908-4165-9368-f70297716e34',
  keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
  time: '1636115048667099257',
};

const devCartsDbLatestServiceEvent = {
  eventId: 'e170b6cc-27ab-4f9a-a8c9-86cb37c2babd',
  keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
  time: '1634037214400569949',
};

const stagingCartsLatestServiceEvent = {
  eventId: '348ad052-dfb8-4bc2-ba88-28d181714d12',
  keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
  time: '1636109592826517241',
};

const stagingCartsDbLatestServiceEvent = {
  eventId: '7e2ba68e-9780-43ca-9aa7-d181329370e6',
  keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
  time: '1634037273207185477',
};

const productionCartsLatestServiceEvent = {
  eventId: 'd9f141c7-4195-4a5d-b555-fb44b2a3ecfc',
  keptnContext: '35383737-3630-4639-b037-353138323631',
  time: '1636002059361084387',
};

const productionCartsDbLatestServiceEvent = {
  eventId: 'aca3bcab-4863-4b87-a1e9-6c328c077348',
  keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
  time: '1634037299899856567',
};

const projectResponseMock: IProject = {
  creationDate: '1632999780800366280',
  gitCredentials: {
    remoteURL: 'https://github.com/Kirdock/keptn-dynatrace-v1',
    user: 'Kirdock',
  },
  projectName: 'sockshop',
  shipyard:
    'apiVersion: spec.keptn.sh/0.2.2\nkind: Shipyard\nmetadata:\n    name: shipyard-sockshop\nspec:\n    stages:\n        - name: dev\n          sequences:\n            - name: delivery\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: direct\n                - name: test\n                  properties:\n                    teststrategy: functional\n                - name: evaluation\n                  properties: null\n                - name: approval\n                  properties:\n                    pass: manual\n                    warning: manual\n                - name: release\n                  properties: null\n            - name: delivery-direct\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: direct\n                - name: approval\n                  properties:\n                    pass: manual\n                    warning: manual\n                - name: release\n                  properties: null\n        - name: staging\n          sequences:\n            - name: delivery\n              triggeredOn:\n                - event: dev.delivery.finished\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: blue_green_service\n                - name: test\n                  properties:\n                    teststrategy: performance\n                - name: evaluation\n                  properties: null\n                - name: approval\n                  properties:\n                    pass: manual\n                    warning: manual\n                - name: release\n                  properties: null\n            - name: rollback\n              triggeredOn:\n                - event: staging.delivery.finished\n                  selector:\n                    match:\n                        result: fail\n              tasks:\n                - name: rollback\n                  properties: null\n            - name: delivery-direct\n              triggeredOn:\n                - event: dev.delivery-direct.finished\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: direct\n                - name: approval\n                  properties:\n                    pass: manual\n                    warning: manual\n                - name: release\n                  properties: null\n        - name: production\n          sequences:\n            - name: delivery\n              triggeredOn:\n                - event: staging.delivery.finished\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: blue_green_service\n                - name: release\n                  properties: null\n            - name: rollback\n              triggeredOn:\n                - event: production.delivery.finished\n                  selector:\n                    match:\n                        result: fail\n              tasks:\n                - name: rollback\n                  properties: null\n            - name: delivery-direct\n              triggeredOn:\n                - event: staging.delivery-direct.finished\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: direct\n                - name: release\n                  properties: null\n            - name: remediation\n              triggeredOn:\n                - event: production.remediation.finished\n                  selector:\n                    match:\n                        evaluation.result: fail\n              tasks:\n                - name: get-action\n                  properties: null\n                - name: action\n                  properties: null\n                - name: evaluation\n                  triggeredAfter: 15m\n                  properties:\n                    timeframe: 15m\n',
  shipyardVersion: 'spec.keptn.sh/0.2.2',
  stages: [
    {
      services: [
        {
          creationDate: '1632999819358104940',
          deployedImage: 'docker.io/keptnexamples/carts:0.12.3',
          lastEventTypes: {
            'sh.keptn.event.approval.finished': {
              eventId: 'a7b4db51-c004-4a16-b4b9-eb290ac64ae8',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636108936755163267',
            },
            'sh.keptn.event.approval.started': {
              eventId: 'b1079092-4c45-4583-abcc-c248528f7dd4',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636115048858051676',
            },
            'sh.keptn.event.approval.triggered': {
              eventId: 'c218f363-4219-45d2-863f-f3fc2329fb16',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636115048772239285',
            },
            'sh.keptn.event.deployment.finished': devCartsDeploymentFinished,
            'sh.keptn.event.deployment.started': {
              eventId: '06c24857-a010-4bdd-bcac-639e6ae7d3ed',
              keptnContext: keptnContextDeploymentDev,
              time: '1636114806860574185',
            },
            'sh.keptn.event.deployment.triggered': {
              eventId: 'ff94e27d-4309-4fa2-8cf3-20f266f9e244',
              keptnContext: keptnContextDeploymentDev,
              time: '1636114806657239737',
            },
            'sh.keptn.event.dev.delivery.finished': {
              eventId: '0f44f059-d2ef-4cbe-875f-7345f90711fb',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636108939659404279',
            },
            'sh.keptn.event.dev.delivery.triggered': {
              eventId: 'a9b2c7dd-f9c0-4f5a-94b4-b0ce60ac97d2',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114805394927785',
            },
            'sh.keptn.event.dev.evaluation.finished': devCartsEvaluationFinished,
            'sh.keptn.event.dev.evaluation.triggered': {
              eventId: 'c4b192e5-e33f-441a-bae8-e0922a80aaa4',
              keptnContext: '79a80508-b9c5-4b71-a8c7-98b39bb4b5dd',
              time: '1635866530701681563',
            },
            'sh.keptn.event.dev.remediation.finished': {
              eventId: '2e607d76-cbcf-4749-9ddf-db55d725f8f3',
              keptnContext: '2d343938-3632-4432-b232-323530393438',
              time: '1636001467063433773',
            },
            'sh.keptn.event.dev.remediation.triggered': {
              eventId: '3a41c9a6-c124-4ea4-8ac6-52db7af92e79',
              keptnContext: '2d343938-3632-4432-b232-323530393438',
              time: '1636001465183051256',
            },
            'sh.keptn.event.evaluation.finished': devCartsLatestServiceEvent,
            'sh.keptn.event.evaluation.started': {
              eventId: '9fa91160-b91d-4980-9b39-9e2aae13466c',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114917956875836',
            },
            'sh.keptn.event.evaluation.triggered': {
              eventId: '6a456cc7-dcaa-4fb5-8c19-f74f0d999ab6',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114917876402210',
            },
            'sh.keptn.event.get-sli.finished': {
              eventId: 'b37c2058-1c19-4a27-996d-3d2256aa40fc',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636115047365792075',
            },
            'sh.keptn.event.get-sli.started': {
              eventId: 'e4e03769-94e7-4ecc-9728-bab6f795de58',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114920461529175',
            },
            'sh.keptn.event.get-sli.triggered': {
              eventId: '06a169b2-cd9e-4e46-a9fc-1b132b42d8de',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114920456530146',
            },
            'sh.keptn.event.release.finished': {
              eventId: 'c23e32de-10d6-4202-b8ad-178234d61628',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636108937459912381',
            },
            'sh.keptn.event.release.started': {
              eventId: 'fe73d937-c4ae-41a1-802d-487cc693aba2',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636108937455992116',
            },
            'sh.keptn.event.release.triggered': {
              eventId: 'b5488157-c683-4ae3-a5a2-fa6773cd4611',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636108937354477796',
            },
            'sh.keptn.event.test.finished': {
              eventId: '47013a83-9bb2-485a-b55c-43db355d6117',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114917763904168',
            },
            'sh.keptn.event.test.started': {
              eventId: 'c822f8a3-1ef0-4768-b525-9c87f932f251',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114894258451746',
            },
            'sh.keptn.event.test.triggered': {
              eventId: '9f98bfaf-35bf-44e1-90c2-f5c7fb740a39',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114894156898359',
            },
            'sh.keptn.events.problem': {
              eventId: '0548ed6a-bc6d-4df7-9623-6f911c94ee7a',
              keptnContext: '2d343938-3632-4432-b232-323530393438',
              time: '1636002113431434060',
            },
          },
          serviceName: 'carts',
        },
        {
          creationDate: '1632999870648452455',
          deployedImage: 'docker.io/mongo:4.2.2',
          lastEventTypes: {
            'sh.keptn.event.approval.finished': {
              eventId: 'e5248059-fa5f-4464-87dc-23bab1edcd8c',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037211852672090',
            },
            'sh.keptn.event.approval.started': {
              eventId: 'b234194d-fae8-470d-bf9e-6a67e338627e',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037198905254590',
            },
            'sh.keptn.event.approval.triggered': {
              eventId: 'd0c26bf3-08a9-43a0-96e7-b14c6a7c981b',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037198900365393',
            },
            'sh.keptn.event.deployment.finished': devCartsDbDeploymentFinished,
            'sh.keptn.event.deployment.started': {
              eventId: 'f4bc1b33-f9fe-4184-aeb1-96a9bff5b9e7',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037176601530855',
            },
            'sh.keptn.event.deployment.triggered': {
              eventId: '666c0833-b726-4b95-b8e2-c7b09cd228a7',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037176504615115',
            },
            'sh.keptn.event.dev.delivery-direct.finished': devCartsDbLatestServiceEvent,
            'sh.keptn.event.dev.delivery-direct.triggered': {
              eventId: 'beed8215-6ad4-4bd9-9405-d0737d1297f4',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037174630495706',
            },
            'sh.keptn.event.release.finished': {
              eventId: '4b8c551b-1c66-461b-bc0b-367b26ae9ebd',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037212106840381',
            },
            'sh.keptn.event.release.started': {
              eventId: '1f8626be-b47e-4967-ad95-a5d7c266e03f',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037212102571106',
            },
            'sh.keptn.event.release.triggered': {
              eventId: 'e109cf77-f3d9-4edb-9428-5269a05d1200',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037212001312487',
            },
          },
          serviceName: 'carts-db',
        },
      ],
      stageName: 'dev',
    },
    {
      services: [
        {
          creationDate: '1632999823491550948',
          deployedImage: 'docker.io/keptnexamples/carts:0.12.3',
          lastEventTypes: {
            'sh.keptn.event.approval.finished': {
              eventId: '0b31d407-db29-4f80-82eb-e30f45a74c1f',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122664254472581',
            },
            'sh.keptn.event.approval.started': {
              eventId: '851442f9-7201-44da-8577-2ab1bf08c476',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122483406081503',
            },
            'sh.keptn.event.approval.triggered': {
              eventId: '7d93986c-37bb-41b0-8e97-2e8ee7eee6c0',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122483400660754',
            },
            'sh.keptn.event.deployment.finished': stagingCartsDeploymentFinished,
            'sh.keptn.event.deployment.started': {
              eventId: '7b473f05-e61e-452a-ba78-4501941a16c6',
              keptnContext: keptnContextDeploymentStaging,
              time: '1636109246055285536',
            },
            'sh.keptn.event.deployment.triggered': {
              eventId: 'baaf784a-34b7-4cc9-bdc5-1b80669b3dde',
              keptnContext: keptnContextDeploymentStaging,
              time: '1636109245959499928',
            },
            'sh.keptn.event.evaluation.finished': stagingCartsEvaluationFinished,
            'sh.keptn.event.evaluation.invalidated': {
              eventId: '6715dd3f-e2c0-4627-aee4-f23ac1f90a78',
              keptnContext: '199287a3-b723-479b-8083-8b3ff3482531',
              time: '1634117655559543662',
            },
            'sh.keptn.event.evaluation.started': {
              eventId: 'b8405eba-1b5a-421b-b9f9-38d95c97d1c3',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109471857303004',
            },
            'sh.keptn.event.evaluation.triggered': {
              eventId: 'd0abb253-5ab5-45ba-84a7-05f9da3ea38f',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109471860859011',
            },
            'sh.keptn.event.get-sli.finished': {
              eventId: 'cad33fc0-00b8-40d2-84ed-059e38966d5d',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109548584193827',
            },
            'sh.keptn.event.get-sli.started': {
              eventId: '963eac9d-d667-437b-8c22-41e36b4ffb51',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109474256595017',
            },
            'sh.keptn.event.get-sli.triggered': {
              eventId: 'b9d70498-fd95-483b-a347-c717e87feb04',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109474170571015',
            },
            'sh.keptn.event.release.finished': {
              eventId: '6430a35c-84d1-4d56-9d66-05ea30d5c9f0',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122784572477487',
            },
            'sh.keptn.event.release.started': {
              eventId: '43278c61-5cb3-408b-bf0c-54b360ff834f',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122664410817094',
            },
            'sh.keptn.event.release.triggered': {
              eventId: 'ddbb13be-918e-498d-9c8a-148693237c23',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122664404659957',
            },
            'sh.keptn.event.rollback.finished': stagingCartsLatestServiceEvent,
            'sh.keptn.event.rollback.started': {
              eventId: '63dc85df-90e8-4cff-b81c-6a23282960d0',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109576258128696',
            },
            'sh.keptn.event.rollback.triggered': {
              eventId: '683b34e8-6dd7-4a62-ae5f-c9f884d45f83',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109576061446957',
            },
            'sh.keptn.event.staging.delivery.finished': {
              eventId: 'f8d890e0-2951-42c4-8b97-d512af6f068a',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109551354602999',
            },
            'sh.keptn.event.staging.delivery.triggered': {
              eventId: '22048580-d042-4f56-b072-fc0d196b2708',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636108939757475937',
            },
            'sh.keptn.event.staging.evaluation.finished': {
              eventId: 'aa55bcb3-7a1f-49ca-806a-c12e73155d81',
              keptnContext: '62acfcc1-02dc-4683-b6f3-64c38f5c534a',
              time: '1635866621511128737',
            },
            'sh.keptn.event.staging.evaluation.triggered': {
              eventId: 'f7e763f3-ea65-4a0b-b833-78f84821af29',
              keptnContext: '62acfcc1-02dc-4683-b6f3-64c38f5c534a',
              time: '1635866599693544147',
            },
            'sh.keptn.event.staging.remediation.finished': {
              eventId: '379c1fa6-4084-419a-b1f1-c0548cdcd65c',
              keptnContext: '33373831-3239-4631-b931-313937323232',
              time: '1633071734110053522',
            },
            'sh.keptn.event.staging.remediation.triggered': {
              eventId: 'eeb50f98-9813-4459-8e77-7718512d0c4f',
              keptnContext: '33373831-3239-4631-b931-313937323232',
              time: '1633071732456666868',
            },
            'sh.keptn.event.staging.rollback.finished': {
              eventId: '236a6da9-d76c-4d6c-bb0c-6e529c6912fb',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109580657320260',
            },
            'sh.keptn.event.staging.rollback.triggered': {
              eventId: 'bb7b26a4-7ade-4da9-a309-6afe8a42add6',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109551360170828',
            },
            'sh.keptn.event.test.finished': {
              eventId: '4b8793a2-17ca-412d-9efe-aa00c88beddd',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109471648146908',
            },
            'sh.keptn.event.test.started': {
              eventId: '15c004f0-9871-42e5-bbd0-345247e3b91e',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109341369573266',
            },
            'sh.keptn.event.test.triggered': {
              eventId: 'ab7009c8-ff2a-48e3-b10b-c160e49c6180',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109341364946130',
            },
            'sh.keptn.events.problem': {
              eventId: 'f43e9b6a-c343-4a10-a016-9b398ef13bc3',
              keptnContext: '33373831-3239-4631-b931-313937323232',
              time: '1633073831796796941',
            },
          },
          serviceName: 'carts',
        },
        {
          creationDate: '1632999875203630913',
          deployedImage: 'docker.io/mongo:4.2.2',
          lastEventTypes: {
            'sh.keptn.event.approval.finished': {
              eventId: 'a823d8b8-146e-442b-94d7-f81870eab073',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037270642778454',
            },
            'sh.keptn.event.approval.started': {
              eventId: '86f1df18-5b05-44d2-940e-cd1fbe0d18b5',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037239503766264',
            },
            'sh.keptn.event.approval.triggered': {
              eventId: '0a6aa4f5-b380-4f25-b8bf-962508013e74',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037239409890718',
            },
            'sh.keptn.event.deployment.finished': stagingCartsDbDeploymentFinished,
            'sh.keptn.event.deployment.started': {
              eventId: 'c8b4f3a7-8ca3-440c-a357-468de68dafd2',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037217005605124',
            },
            'sh.keptn.event.deployment.triggered': {
              eventId: '9f3457ea-118e-4650-af7e-f973b9494066',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037216905851839',
            },
            'sh.keptn.event.release.finished': {
              eventId: 'e243bbd2-fd76-48e0-a102-d507543e5bab',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037270902785621',
            },
            'sh.keptn.event.release.started': {
              eventId: 'f38c3a09-0938-4de4-af1a-f6abd6910b44',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037271006072695',
            },
            'sh.keptn.event.release.triggered': {
              eventId: 'da0a45a3-bea4-458e-be2e-62710fe6c110',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037270808094764',
            },
            'sh.keptn.event.staging.delivery-direct.finished': stagingCartsDbLatestServiceEvent,
            'sh.keptn.event.staging.delivery-direct.triggered': {
              eventId: 'b1e042b9-d38b-48e0-87de-d57892b5412b',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037214405956780',
            },
          },
          serviceName: 'carts-db',
        },
      ],
      stageName: 'staging',
      parentStages: ['dev'],
    },
    {
      services: [
        {
          creationDate: '1632999827864626575',
          deployedImage: 'docker.io/keptnexamples/carts:0.12.1',
          lastEventTypes: {
            'sh.keptn.event.deployment.finished': productionCartsDeploymentFinished,
            'sh.keptn.event.deployment.started': {
              eventId: 'bef96141-19c6-473d-8eda-468f8d4f7dce',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122786906836895',
            },
            'sh.keptn.event.deployment.triggered': {
              eventId: '983e3fb5-c868-4fcc-90d2-5724b499826a',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122786902371806',
            },
            'sh.keptn.event.get-action.finished': {
              eventId: '928f8393-c5c9-41f7-8179-5c49fc4092a6',
              keptnContext: '35383737-3630-4639-b037-353138323631',
              time: '1636001483077646864',
            },
            'sh.keptn.event.get-action.started': {
              eventId: '7d580047-261c-43e8-886d-17a5ae5bdcc5',
              keptnContext: '35383737-3630-4639-b037-353138323631',
              time: '1636001481758951700',
            },
            'sh.keptn.event.get-action.triggered': {
              eventId: 'ebf4f033-5cb6-49c1-897d-22c9851417ad',
              keptnContext: '35383737-3630-4639-b037-353138323631',
              time: '1636001481667948504',
            },
            'sh.keptn.event.production.delivery.finished': {
              eventId: '46ec4f2d-21c4-49bd-94c5-3976c1808abc',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122992304654740',
            },
            'sh.keptn.event.production.delivery.triggered': {
              eventId: '61a2d8f9-7368-4097-b469-2fb81af50eb3',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122785401791864',
            },
            'sh.keptn.event.production.remediation.finished': {
              eventId: '7448420f-5b15-4777-9d39-cc8308e2b0c3',
              keptnContext: '35383737-3630-4639-b037-353138323631',
              time: '1636001483355675294',
            },
            'sh.keptn.event.production.remediation.triggered': {
              eventId: 'f17573df-4efb-4143-9aa9-a41169920cf7',
              keptnContext: '35383737-3630-4639-b037-353138323631',
              time: '1636001480334972446',
            },
            'sh.keptn.event.release.finished': {
              eventId: '934c5141-a130-4aa7-9cba-00c162309524',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122992132898394',
            },
            'sh.keptn.event.release.started': {
              eventId: '0049ede6-11a7-4146-8b0e-8fd718595a64',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122879103916639',
            },
            'sh.keptn.event.release.triggered': {
              eventId: '3926781b-9cb0-4538-938d-f3f6dfdd46d4',
              keptnContext: '2c0e568b-8bd3-4726-a188-e528423813ed',
              time: '1634122878705431780',
            },
            'sh.keptn.events.problem': productionCartsLatestServiceEvent,
          },
          serviceName: 'carts',
        },
        {
          creationDate: '1632999879474188928',
          deployedImage: 'docker.io/mongo:4.2.2',
          lastEventTypes: {
            'sh.keptn.event.deployment.finished': productionCartsDbDeploymentFinished,
            'sh.keptn.event.deployment.started': {
              eventId: '16c31a67-96cf-4189-afd8-c78b60259598',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037276001777741',
            },
            'sh.keptn.event.deployment.triggered': {
              eventId: '9a0ece18-1679-4d2f-a294-6208baad945b',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037275909445564',
            },
            'sh.keptn.event.production.delivery-direct.finished': productionCartsDbLatestServiceEvent,
            'sh.keptn.event.production.delivery-direct.triggered': {
              eventId: '0f700a4d-438d-495d-9f18-066438e3debc',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037273300865200',
            },
            'sh.keptn.event.release.finished': {
              eventId: 'ec573c8e-7ac0-4f88-849a-c36e165ef24b',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037297901344935',
            },
            'sh.keptn.event.release.started': {
              eventId: 'c3631d56-5bea-4d18-a8ee-c8359db2386e',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037297703401647',
            },
            'sh.keptn.event.release.triggered': {
              eventId: '01b2dfc4-ddda-4ba8-8583-cab5be88f1bc',
              keptnContext: '0cc574e9-3d47-4a29-81b7-84faf33bdc9c',
              time: '1634037297604313167',
            },
          },
          serviceName: 'carts-db',
        },
      ],
      stageName: 'production',
      parentStages: ['staging'],
    },
  ],
};

const projectQualityGatesResponse: IProject = {
  creationDate: '1632999780800366280',
  gitCredentials: {
    remoteURL: 'https://github.com/Kirdock/keptn-dynatrace-v1',
    user: 'Kirdock',
  },
  projectName: 'sockshop',
  shipyard:
    'apiVersion: spec.keptn.sh/0.2.2\nkind: Shipyard\nmetadata:\n    name: shipyard-sockshop\nspec:\n    stages:\n        - name: dev\n          sequences:\n            - name: delivery\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: direct\n                - name: test\n                  properties:\n                    teststrategy: functional\n                - name: evaluation\n                  properties: null\n                - name: approval\n                  properties:\n                    pass: manual\n                    warning: manual\n                - name: release\n                  properties: null\n            - name: delivery-direct\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: direct\n                - name: approval\n                  properties:\n                    pass: manual\n                    warning: manual\n                - name: release\n                  properties: null\n        - name: staging\n          sequences:\n            - name: delivery\n              triggeredOn:\n                - event: dev.delivery.finished\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: blue_green_service\n                - name: test\n                  properties:\n                    teststrategy: performance\n                - name: evaluation\n                  properties: null\n                - name: approval\n                  properties:\n                    pass: manual\n                    warning: manual\n                - name: release\n                  properties: null\n            - name: rollback\n              triggeredOn:\n                - event: staging.delivery.finished\n                  selector:\n                    match:\n                        result: fail\n              tasks:\n                - name: rollback\n                  properties: null\n            - name: delivery-direct\n              triggeredOn:\n                - event: dev.delivery-direct.finished\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: direct\n                - name: approval\n                  properties:\n                    pass: manual\n                    warning: manual\n                - name: release\n                  properties: null\n        - name: production\n          sequences:\n            - name: delivery\n              triggeredOn:\n                - event: staging.delivery.finished\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: blue_green_service\n                - name: release\n                  properties: null\n            - name: rollback\n              triggeredOn:\n                - event: production.delivery.finished\n                  selector:\n                    match:\n                        result: fail\n              tasks:\n                - name: rollback\n                  properties: null\n            - name: delivery-direct\n              triggeredOn:\n                - event: staging.delivery-direct.finished\n              tasks:\n                - name: deployment\n                  properties:\n                    deploymentstrategy: direct\n                - name: release\n                  properties: null\n            - name: remediation\n              triggeredOn:\n                - event: production.remediation.finished\n                  selector:\n                    match:\n                        evaluation.result: fail\n              tasks:\n                - name: get-action\n                  properties: null\n                - name: action\n                  properties: null\n                - name: evaluation\n                  triggeredAfter: 15m\n                  properties:\n                    timeframe: 15m\n',
  shipyardVersion: 'spec.keptn.sh/0.2.2',
  stages: [
    {
      services: [
        {
          creationDate: '1632999819358104940',
          lastEventTypes: {
            'sh.keptn.event.dev.evaluation.finished': {
              eventId: '16338541-30a0-4a6b-b990-d4e81d0dd9fd',
              keptnContext: '79a80508-b9c5-4b71-a8c7-98b39bb4b5dd',
              time: '1635866557202816616',
            },
            'sh.keptn.event.dev.evaluation.triggered': {
              eventId: 'c4b192e5-e33f-441a-bae8-e0922a80aaa4',
              keptnContext: '79a80508-b9c5-4b71-a8c7-98b39bb4b5dd',
              time: '1635866530701681563',
            },
            'sh.keptn.event.evaluation.finished': {
              eventId: '1064035a-a908-4165-9368-f70297716e34',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636115048667099257',
            },
            'sh.keptn.event.evaluation.started': {
              eventId: '9fa91160-b91d-4980-9b39-9e2aae13466c',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114917956875836',
            },
            'sh.keptn.event.evaluation.triggered': {
              eventId: '6a456cc7-dcaa-4fb5-8c19-f74f0d999ab6',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114917876402210',
            },
            'sh.keptn.event.get-sli.finished': {
              eventId: 'b37c2058-1c19-4a27-996d-3d2256aa40fc',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636115047365792075',
            },
            'sh.keptn.event.get-sli.started': {
              eventId: 'e4e03769-94e7-4ecc-9728-bab6f795de58',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114920461529175',
            },
            'sh.keptn.event.get-sli.triggered': {
              eventId: '06a169b2-cd9e-4e46-a9fc-1b132b42d8de',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114920456530146',
            },
            'sh.keptn.event.test.finished': {
              eventId: '47013a83-9bb2-485a-b55c-43db355d6117',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114917763904168',
            },
            'sh.keptn.event.test.started': {
              eventId: 'c822f8a3-1ef0-4768-b525-9c87f932f251',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114894258451746',
            },
            'sh.keptn.event.test.triggered': {
              eventId: '9f98bfaf-35bf-44e1-90c2-f5c7fb740a39',
              keptnContext: '2e21574c-dcf7-4275-b677-6bc19214acd5',
              time: '1636114894156898359',
            },
          },
          serviceName: 'carts',
        },
        {
          creationDate: '1632999870648452455',
          lastEventTypes: {},
          serviceName: 'carts-db',
        },
      ],
      stageName: 'dev',
    },
    {
      services: [
        {
          creationDate: '1632999823491550948',
          lastEventTypes: {
            'sh.keptn.event.evaluation.finished': {
              eventId: '7c1e8531-3005-4b4f-831e-da84d3613f93',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109551167902030',
            },
            'sh.keptn.event.evaluation.invalidated': {
              eventId: '6715dd3f-e2c0-4627-aee4-f23ac1f90a78',
              keptnContext: '199287a3-b723-479b-8083-8b3ff3482531',
              time: '1634117655559543662',
            },
            'sh.keptn.event.evaluation.started': {
              eventId: 'b8405eba-1b5a-421b-b9f9-38d95c97d1c3',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109471857303004',
            },
            'sh.keptn.event.evaluation.triggered': {
              eventId: 'd0abb253-5ab5-45ba-84a7-05f9da3ea38f',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109471860859011',
            },
            'sh.keptn.event.get-sli.finished': {
              eventId: 'cad33fc0-00b8-40d2-84ed-059e38966d5d',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109548584193827',
            },
            'sh.keptn.event.get-sli.started': {
              eventId: '963eac9d-d667-437b-8c22-41e36b4ffb51',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109474256595017',
            },
            'sh.keptn.event.get-sli.triggered': {
              eventId: 'b9d70498-fd95-483b-a347-c717e87feb04',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109474170571015',
            },
            'sh.keptn.event.staging.evaluation.finished': {
              eventId: 'aa55bcb3-7a1f-49ca-806a-c12e73155d81',
              keptnContext: '62acfcc1-02dc-4683-b6f3-64c38f5c534a',
              time: '1635866621511128737',
            },
            'sh.keptn.event.staging.evaluation.triggered': {
              eventId: 'f7e763f3-ea65-4a0b-b833-78f84821af29',
              keptnContext: '62acfcc1-02dc-4683-b6f3-64c38f5c534a',
              time: '1635866599693544147',
            },
            'sh.keptn.event.test.finished': {
              eventId: '4b8793a2-17ca-412d-9efe-aa00c88beddd',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109471648146908',
            },
            'sh.keptn.event.test.started': {
              eventId: '15c004f0-9871-42e5-bbd0-345247e3b91e',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109341369573266',
            },
            'sh.keptn.event.test.triggered': {
              eventId: 'ab7009c8-ff2a-48e3-b10b-c160e49c6180',
              keptnContext: '29af69cc-ea85-4358-b169-ce29034d9c81',
              time: '1636109341364946130',
            },
          },
          serviceName: 'carts',
        },
        {
          creationDate: '1632999875203630913',
          lastEventTypes: {},
          serviceName: 'carts-db',
        },
      ],
      stageName: 'staging',
      parentStages: ['dev'],
    },
    {
      services: [
        {
          creationDate: '1632999827864626575',
          lastEventTypes: {},
          serviceName: 'carts',
        },
        {
          creationDate: '1632999879474188928',
          lastEventTypes: {},
          serviceName: 'carts-db',
        },
      ],
      stageName: 'production',
      parentStages: ['staging'],
    },
  ],
};

const projectResponseURLFallback: IProject = {
  projectName: 'sockshop',
  shipyard: '',
  creationDate: '',
  shipyardVersion: '',
  stages: [
    {
      stageName: 'dev',
      services: [
        {
          serviceName: 'carts',
          creationDate: '',
          lastEventTypes: {
            [EventTypes.DEPLOYMENT_FINISHED]: {
              eventId: 'eventId',
              keptnContext: 'keptnContext',
              time: '1637666066861000',
            },
          },
        },
      ],
    },
  ],
};

const projectResponseEvaluationFallback: IProject = {
  projectName: 'sockshop',
  shipyard: '',
  creationDate: '',
  shipyardVersion: '',
  stages: [
    {
      stageName: 'dev',
      services: [
        {
          serviceName: 'carts',
          creationDate: '',
          lastEventTypes: {
            [EventTypes.EVALUATION_FINISHED]: {
              eventId: 'webhookId',
              keptnContext: 'keptnContext',
              time: '1637666066861000',
            },
          },
        },
      ],
    },
  ],
};

const projectResponseIntersect: IProject = {
  projectName: 'sockshop',
  shipyard: '',
  creationDate: '',
  shipyardVersion: '',
  stages: [
    {
      stageName: 'dev',
      services: [
        {
          serviceName: 'carts',
          creationDate: '',
          lastEventTypes: {
            [EventTypes.DEPLOYMENT_TRIGGERED]: {
              eventId: '1',
              keptnContext: 'context1',
              time: '1637666066861000',
            },
            [EventTypes.DEPLOYMENT_STARTED]: {
              eventId: '2',
              keptnContext: 'context2',
              time: '1637666066861000',
            },
            [EventTypes.DEPLOYMENT_FINISHED]: {
              eventId: '3',
              keptnContext: 'context3',
              time: '1637666066861000',
            },
          },
        },
        {
          serviceName: 'carts-db',
          creationDate: '',
          lastEventTypes: {
            [EventTypes.DEPLOYMENT_TRIGGERED]: {
              eventId: '4',
              keptnContext: 'context1',
              time: '1637666066861000',
            },
            [EventTypes.DEPLOYMENT_STARTED]: {
              eventId: '5',
              keptnContext: 'context2',
              time: '1637666066861000',
            },
            [EventTypes.DEPLOYMENT_FINISHED]: {
              eventId: '6',
              keptnContext: 'context3',
              time: '1637666066861000',
            },
          },
        },
      ],
    },
    {
      stageName: 'staging',
      services: [
        {
          serviceName: 'carts',
          creationDate: '',
          lastEventTypes: {
            [EventTypes.DEPLOYMENT_TRIGGERED]: {
              eventId: '7',
              keptnContext: 'context1',
              time: '1637666066861000',
            },
            [EventTypes.DEPLOYMENT_STARTED]: {
              eventId: '8',
              keptnContext: 'context2',
              time: '1637666066861000',
            },
            [EventTypes.DEPLOYMENT_FINISHED]: {
              eventId: '9',
              keptnContext: 'context3',
              time: '1637666066861000',
            },
          },
        },
        {
          serviceName: 'carts-db',
          creationDate: '',
          lastEventTypes: {
            [EventTypes.DEPLOYMENT_TRIGGERED]: {
              eventId: '10',
              keptnContext: 'context1',
              time: '1637666066861000',
            },
            [EventTypes.DEPLOYMENT_STARTED]: {
              eventId: '11',
              keptnContext: 'context2',
              time: '1637666066861000',
            },
          },
        },
      ],
    },
  ],
};

const projectDetailsResponseURLFallback = {
  stages: [
    {
      services: [
        {
          creationDate: '',
          lastEventTypes: {
            'sh.keptn.event.deployment.finished': {
              eventId: 'eventId',
              keptnContext: 'keptnContext',
              time: '1637666066861000',
            },
          },
          openRemediations: [],
          openApprovals: [],
          serviceName: 'carts',
          latestSequence: {
            name: 'delivery',
            service: 'carts',
            project: 'sockshop',
            time: '2021-11-05T12:20:06.463Z',
            shkeptncontext: 'keptnContext',
            state: 'finished',
            stages: [
              {
                name: 'dev',
                state: 'finished',
                latestEvent: {
                  type: 'sh.keptn.event.deployment.finished',
                  id: 'eventId',
                  time: '2021-11-10T11:59:58.655Z',
                },
                latestFailedEvent: {
                  type: 'sh.keptn.event.deployment.finished',
                  id: 'eventId',
                  time: '2021-11-10T11:59:58.655Z',
                },
              },
            ],
          },
          deploymentInformation: { deploymentUrl: 'http://carts.sockshop-dev.35.192.209.116.nip.io:80' },
        },
      ],
      stageName: 'dev',
    },
  ],
  projectName: 'sockshop',
  shipyard: '',
  creationDate: '',
  shipyardVersion: '',
};

const projectDetailsResponseEvaluationFallback = {
  stages: [
    {
      services: [
        {
          creationDate: '',
          lastEventTypes: {
            'sh.keptn.event.evaluation.finished': {
              eventId: 'webhookId',
              keptnContext: 'keptnContext',
              time: '1637666066861000',
            },
          },
          openRemediations: [],
          openApprovals: [],
          serviceName: 'carts',
          latestSequence: {
            name: 'delivery',
            service: 'carts',
            project: 'sockshop',
            time: '2021-11-05T12:20:06.463Z',
            shkeptncontext: 'keptnContext',
            state: 'finished',
            stages: [
              {
                name: 'dev',
                state: 'finished',
                latestEvent: {
                  type: 'sh.keptn.event.evaluation.finished',
                  id: 'eventId',
                  time: '2021-11-10T11:59:58.655Z',
                },
                latestEvaluationTrace: {
                  data: {
                    evaluation: {
                      comparedEvents: ['3344487d-e384-4cd9-a0e0-fcf157a33ad6'],
                      gitCommit: '',
                      indicatorResults: [
                        {
                          displayName: 'Response time P95',
                          keySli: false,
                          passTargets: [
                            { criteria: '<=+10%', targetValue: 336.9946150194969, violated: false },
                            { criteria: '<600', targetValue: 600, violated: false },
                          ],
                          score: 1,
                          status: 'pass',
                          value: { metric: 'response_time_p95', success: true, value: 304.2952915485157 },
                          warningTargets: [{ criteria: '<=800', targetValue: 800, violated: false }],
                        },
                      ],
                      result: 'pass',
                      score: 100,
                      sloFileContent:
                        '',
                      timeEnd: '2021-10-13T10:53:29Z',
                      timeStart: '2021-10-13T10:51:08Z',
                    },
                    labels: { DtCreds: 'dynatrace' },
                    project: 'sockshop',
                    result: 'pass',
                    service: 'carts',
                    stage: 'dev',
                    status: 'succeeded',
                    temporaryData: { distributor: { subscriptionID: '' } },
                  },
                  id: '93c2eba9-b77c-4976-b079-29a0188d86ef',
                  source: 'lighthouse-service',
                  specversion: '1.0',
                  time: '2021-10-13T10:54:43.112Z',
                  type: 'sh.keptn.event.evaluation.finished',
                  shkeptncontext: '2c0e568b-8bd3-4726-a188-e528423813ed',
                  shkeptnspecversion: '0.2.3',
                  triggeredid: '1cc9c272-721a-43de-98f6-9eceae484cf5',
                },
              },
            ],
          },
        },
      ],
      stageName: 'dev',
    },
  ],
  projectName: 'sockshop',
  shipyard: '',
  creationDate: '',
  shipyardVersion: '',
};

export { projectResponseMock as ProjectResponse };
export { projectResponseIntersect as ProjectResponseIntersect };
export { projectResponseURLFallback as ProjectResponseURLFallback };
export { projectQualityGatesResponse as ProjectQualityGatesResponse };
export { projectResponseEvaluationFallback as ProjectResponseEvaluationFallback };
export { projectDetailsResponseURLFallback as ProjectDetailsResponseURLFallback };
export { projectDetailsResponseEvaluationFallback as ProjectDetailsResponseEvaluationFallback };
export { devCartsDeploymentFinished as DevCartsDeploymentFinished };
export { devCartsDbDeploymentFinished as DevCartsDbDeploymentFinished };
export { devCartsEvaluationFinished as DevCartsEvaluationFinished };
export { stagingCartsDeploymentFinished as StagingCartsDeploymentFinished };
export { stagingCartsDbDeploymentFinished as StagingCartsDbDeploymentFinished };
export { stagingCartsEvaluationFinished as StagingCartsEvaluationFinished };
export { productionCartsDeploymentFinished as ProductionCartsDeploymentFinished };
export { productionCartsDbDeploymentFinished as ProductionCartsDbDeploymentFinished };
export { devCartsLatestServiceEvent as DevCartsLatestServiceEvent };
export { devCartsDbLatestServiceEvent as DevCartsDbLatestServiceEvent };
export { stagingCartsLatestServiceEvent as StagingCartsLatestServiceEvent };
export { stagingCartsDbLatestServiceEvent as StagingCartsDbLatestServiceEvent };
export { productionCartsLatestServiceEvent as ProductionCartsLatestServiceEvent };
export { productionCartsDbLatestServiceEvent as ProductionCartsDbLatestServiceEvent };
