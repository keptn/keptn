...
class SecretsPage{
    ...
    addSecret(SECRET_NAME, SECRET_KEY, SECRET_VALUE){
        this.clickAddSecret()
        cy.get('input[placeholder="Secret name"]').type(SECRET_NAME)
            .get('input[placeholder="Key"]').type(SECRET_KEY)
                .get('input[placeholder="Value"]').type(SECRET_VALUE)
                    .get('button > span').contains('Add secret').click()
