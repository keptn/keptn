...
func TransformEvaluationDoneEvent(keptnEvent models.KeptnContextExtendedCE) (*models.KeptnContextExtendedCE, error) {
	eventMap := map[string]interface{}{}
	convertedEvent := &keptnv2.EvaluationFinishedEventData{}
	if err := keptnv2.Decode(keptnEvent.Data, &eventMap); err != nil {
		return nil, fmt.Errorf("failed to transform evaluation-done event to evaluation.finished event %v", err)
	}
	...
	if eventMap["evaluationdetails"] != nil {
		...
		if err := keptnv2.Decode(eventMap["evaluationdetails"], evaluationDetails); err != nil {
			return nil, fmt.Errorf("failed to transform evaluationDetails of evaluation-done event: %v", err)
		}
