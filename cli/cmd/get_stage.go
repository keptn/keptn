...
var getStageCmd = &cobra.Command{
	...
	Long:  `Get details of a stage from a given keptn project`,
	Example: `keptn get stage staging --project sockshop
NAME           CREATION DATE                 
staging        2020-04-06T14:37:45.210Z
	...
	Args: func(cmd *cobra.Command, args []string) error {
		...
		if len(args) != 1 {
			...
			return errors.New("required argument STAGENAME not set")
		}
		return nil
	},
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !mocking {
			...
			fmt.Println("NAME\t\tCREATION DATE")
			for _, stage := range stages {
				...
				if stage.StageName == stageName {
					fmt.Println(stage.StageName + "\tn/a")
				}
