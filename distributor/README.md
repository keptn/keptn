...
- `API_PROXY_MAX_PAYLOAD_BYTES_KB` - Maximum request body size in kilobytes for requests sent via the distributor's API proxy. default = `64`.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: distributor
          image: ghcr.io/keptn/distributor:latest
          ports:
            - containerPort: 8080
          resources:
            requests:
