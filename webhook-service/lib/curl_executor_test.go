...
func TestCmdCurlExecutor_Curl(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid request - append --fail flag",
			args: args{
			...
			shouldExecute: true,
			wantPassedArgs: []string{
				"-X", "POST", "-H", "Content-type: application/json", "--data", `{\"text\":\"Hello, World!\"}`, "https://my.hook.com/foo", "--fail",
			},
			wantErr: false,
		},
		{
			name: "valid request - --fail flag already there",
			args: args{
				curlCmd: `curl -X POST -H 'Content-type: application/json' --data '{\"text\":\"Hello, World!\"}' https://my.hook.com/foo --fail`,
			},
			want:          "success",
			shouldExecute: true,
			wantPassedArgs: []string{
