...
func TestApiSetInternalMappings(t *testing.T) {
	t.Run("TestInternalAPISet - Default API Mappings", func(t *testing.T) {
		...
		assert.Equal(t, DefaultInClusterAPIMappings[ApiService], internal.APIV1().(*InternalAPIHandler).apiServiceApiHandler.BaseURL)
		assert.Equal(t, DefaultInClusterAPIMappings[ShipyardController], internal.APIV1().(*InternalAPIHandler).shippyApiHandler.BaseURL)
	...
	t.Run("TestInternalAPISet - Override Mappings", func(t *testing.T) {
		...
		assert.Equal(t, overrideMappings[ApiService], internal.APIV1().(*InternalAPIHandler).apiServiceApiHandler.BaseURL)
		assert.Equal(t, overrideMappings[ShipyardController], internal.APIV1().(*InternalAPIHandler).shippyApiHandler.BaseURL)
