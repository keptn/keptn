...
describe('DockerService', () => {
  ...
  beforeEach(() => {
    ...
    dockerService = new DockerService(messageService, orgToRepoMapper);
  });
  ...
  it('should return false if no repo could be found', async () => {
    const message: DockerRequestModel = {
      events: [
        {
          id: 'a24e1fe3-efc9-42e3-b274-3c736f015552',
    ...
    const messageServiceSendMessageStub = sinon.spy();
    messageService.sendMessage = messageServiceSendMessageStub;

    const orgToRepoStub = sinon.stub().resolves('');
    orgToRepoMapper.getRepoForOrg = orgToRepoStub;

    const result = await dockerService.handleDockerRequest(message);
    expect(messageServiceSendMessageStub.called).to.be.false;
    expect(result).to.be.false;
  });
