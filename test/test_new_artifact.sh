...
keptn send event new-artifact --project=$PROJECT --service=carts-db --image=mongo --stage=dev
...
test/utils/send_new_artifact_sockshop.sh $PROJECT docker.io/keptnexamples/carts 0.10.1 --stage=dev
...
test/utils/send_new_artifact_sockshop.sh $PROJECT docker.io/keptnexamples/carts 0.10.3 --stage=dev
