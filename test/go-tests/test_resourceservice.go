...
func TestResourceServiceBasic(t *testing.T) {
	...
	resp, err = ApiPOSTRequest(configServiceUrlFragment+projectName+resourceUrlFragment, createResourceRequest, 3)
	require.Nil(t, err)
	require.Equal(t, 201, resp.Response().StatusCode)

	t.Logf("Checking resource for project %s", projectName)
	resp, err = ApiGETRequest(configServiceUrlFragment+projectName+resourceUrlFragment+resourceUriPath, 3)
	require.Nil(t, err)
	require.Equal(t, 200, resp.Response().StatusCode)

	t.Logf(checkBodyLogString)
	...
	resp, err = ApiGETRequest(configServiceUrlFragment+projectName+resourceUrlFragment, 3)
	require.Nil(t, err)
	require.Equal(t, 200, resp.Response().StatusCode)
	...
	resp, err = ApiGETRequest(configServiceUrlFragment+nonExistingProjectName+resourceUrlFragment, 3)
	require.Nil(t, err)
	...
	resp, err = ApiPOSTRequest(configServiceUrlFragment+nonExistingProjectName+resourceUrlFragment, createResourceRequest, 3)
	require.Nil(t, err)
	require.Equal(t, 400, resp.Response().StatusCode)

	t.Logf("Creating a new resource with invalid payload for project %s", projectName)
	resp, err = ApiPOSTRequest(configServiceUrlFragment+projectName+resourceUrlFragment, invalidResourceRequest, 3)
	require.Nil(t, err)
	require.Equal(t, 400, resp.Response().StatusCode)

	for _, stageReq := range createStageRequests {
		t.Logf("Creating a new stage %s in project %s", stageReq.StageName, projectName)
		resp, err = ApiPOSTRequest(configServiceUrlFragment+projectName+"/stage", stageReq, 3)
		require.Nil(t, err)
		require.Equal(t, 204, resp.Response().StatusCode)
	...
	resp, err = ApiPUTRequest(configServiceUrlFragment+projectName, updateProjectRequest, 3)
	require.Nil(t, err)
	require.Equal(t, 204, resp.Response().StatusCode)

	t.Logf("Updating existing resource of project %s", projectName)
	resp, err = ApiPUTRequest(configServiceUrlFragment+projectName+resourceUrlFragment+resourceUriPath, updateResourceRequest, 3)
	require.Nil(t, err)
	require.Equal(t, 201, resp.Response().StatusCode)
	...
	resp, err = ApiPUTRequest(configServiceUrlFragment+projectName+resourceUrlFragment, updateResourceListRequest, 3)
	require.Nil(t, err)
	require.Equal(t, 201, resp.Response().StatusCode)
	...
	resp, err = ApiPUTRequest(configServiceUrlFragment+projectName+resourceUrlFragment+resourceUriPath, invalidResourceRequest, 3)
	require.Nil(t, err)
	require.Equal(t, 400, resp.Response().StatusCode)

	t.Logf("Updating existing list of resources with invalid payload of project %s", projectName)
	resp, err = ApiPUTRequest(configServiceUrlFragment+projectName+resourceUrlFragment, invalidResourceRequest, 3)
	require.Nil(t, err)
	require.Equal(t, 400, resp.Response().StatusCode)
	...
	resp, err = ApiDELETERequest(configServiceUrlFragment+projectName+resourceUrlFragment+resourceUriPath, 3)
	require.Nil(t, err)
	require.Equal(t, 204, resp.Response().StatusCode)
	...
	resp, err = ApiDELETERequest(configServiceUrlFragment+projectName, 3)
	require.Nil(t, err)
	require.Equal(t, 204, resp.Response().StatusCode)
	...
	resp, err = ApiDELETERequest(configServiceUrlFragment+nonExistingProjectName, 3)
	require.Nil(t, err)
	require.Equal(t, 204, resp.Response().StatusCode)
