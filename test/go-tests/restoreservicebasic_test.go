...
func Test_RestoreServiceBasic(t *testing.T) {
	...
	//vyskusaj tu aj duplicitne vytvorenie objektov aj resourcov
	...
	resp, err = ApiGETRequest("/configuration-service/v1/project/"+projectName+"/resource/load.jmx", 3)
	require.Nil(t, err)
	...
	t.Logf("Creating a new resource for non-existing project %s", projectName+"22")
	resp, err = ApiPOSTRequest("/configuration-service/v1/project/"+projectName+"22/resource", createResourceRequest, 3)
	require.Nil(t, err)
	require.Equal(t, 400, resp.Response().StatusCode)
	...
	for _, stageReq := range createStageRequests {
		...
		resp, err = ApiGETRequest("/configuration-service/v1/project/"+projectName+"/stage/"+stageReq.StageName+"/resource/load.jmx", 3)
		require.Nil(t, err)
		require.Equal(t, 200, resp.Response().StatusCode)
		...
		t.Logf("Creating a new resource for non-existing stage %s for project %s", stageReq.StageName+"22", projectName)
		resp, err = ApiPOSTRequest("/configuration-service/v1/project/"+projectName+"/stage/"+stageReq.StageName+"22/resource", createResourceRequest, 3)
		require.Nil(t, err)
		require.Equal(t, 400, resp.Response().StatusCode)
	}
	...
	for _, stageReq := range createStageRequests {
		for _, serviceReq := range createServiceRequests {
			...
			resp, err = ApiGETRequest("/configuration-service/v1/project/"+projectName+"/stage/"+stageReq.StageName+"/service/"+serviceReq.ServiceName+"/resource/load.jmx", 3)
			require.Nil(t, err)
			require.Equal(t, 200, resp.Response().StatusCode)
			...
			t.Logf("Creating a new resource for non-existing service %s in stage %s for project %s", serviceReq.ServiceName+"22", stageReq.StageName, projectName)
			resp, err = ApiPOSTRequest("/configuration-service/v1/project/"+projectName+"/stage/"+stageReq.StageName+"/service/"+serviceReq.ServiceName+"22/resource", createResourceRequest, 3)
			require.Nil(t, err)
			require.Equal(t, 400, resp.Response().StatusCode)
		}
	}
	...
	for _, stageReq := range createStageRequests {
		for _, serviceReq := range createServiceRequests {
			...
			resp, err = ApiDELETERequest("/configuration-service/v1/project/"+projectName+"/stage/"+stageReq.StageName+"/service/"+serviceReq.ServiceName+"/resource/load.jmx", 3)
			require.Nil(t, err)
			require.Equal(t, 204, resp.Response().StatusCode)

			t.Logf("Checking resource for service %s for stage %s for project %s", serviceReq.ServiceName, stageReq.StageName, projectName)
			resp, err = ApiGETRequest("/configuration-service/v1/project/"+projectName+"/stage/"+stageReq.StageName+"/service/"+serviceReq.ServiceName+"/resource/load.jmx", 3)
			require.Nil(t, err)
	...
	for _, stageReq := range createStageRequests {
		...
		resp, err = ApiDELETERequest("/configuration-service/v1/project/"+projectName+"/stage/"+stageReq.StageName+"/resource/load.jmx", 3)
		require.Nil(t, err)
		require.Equal(t, 204, resp.Response().StatusCode)
	...
	resp, err = ApiDELETERequest("/configuration-service/v1/project/"+projectName+"/resource/load.jmx", 3)
	require.Nil(t, err)
	require.Equal(t, 204, resp.Response().StatusCode)
