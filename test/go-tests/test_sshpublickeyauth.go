...
func Test_SSHPublicKeyAuth(t *testing.T) {
	...
	_, err = ExecuteCommand(fmt.Sprintf("keptn update project keptn-%s --git-remote-url=ssh://gitea-ssh:22/%s/keptn-%s.git --git-user=%s --git-private-key=%s --git-private-key-pass=%s", projectName, user, projectName, user, privateKeyPath, passphrase))
	require.Nil(t, err)
