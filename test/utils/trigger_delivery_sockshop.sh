...
DEV_URL="http://carts.${PROJECT}-dev.$(kubectl get cm ingress-config -n "${KEPTN_NAMESPACE}" -o=jsonpath='{.data.ingress_hostname_suffix}')"
...
# get URL for that deployment
STAGING_URL="http://carts.${PROJECT}-staging.$(kubectl get cm ingress-config -n "${KEPTN_NAMESPACE}" -o=jsonpath='{.data.ingress_hostname_suffix}')"
# try to access that URL
...
# get URL for that deployment
PRODUCTION_URL="http://carts.${PROJECT}-production.$(kubectl get cm ingress-config -n "${KEPTN_NAMESPACE}" -o=jsonpath='{.data.ingress_hostname_suffix}')"
# try to access that URL
